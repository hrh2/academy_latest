...
export function SignIn() {
  ...
  const handleOnSubmit = async (event) => {
    ...
    setLoader(true);
    try {
      const response = await axios.post(`${servers.main_api}/login`, data);
      const token = response.data.token;
      localStorage.setItem('buu_client_token', token);
  ...
  return (
    <section className="h-screen p-2 flex overflow-hidden">
      <div className="w-full  h-full  lg:w-3/5 flex flex-col items-center justify-center overflow-y-scroll pt-24">
        <div className="text-center">
          <Typography variant="h2" className="font-bold mb-4">Sign In</Typography>
          <Typography variant="paragraph" color="blue-gray" className="text-lg font-normal">Enter your email and password to Sign In.</Typography>
        </div>
        <div className="space-y-4 pt-8 mx-auto w-80 max-w-screen-lg lg:w-1/2">
        ...
        <form onSubmit={handleOnSubmit} className="pt-8 pb-4 mx-auto w-80 max-w-screen-lg lg:w-1/2">
